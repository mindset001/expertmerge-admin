...
import { getDeletedUsers } from '@/app/api/services/endpoints/content';
...
export default function TotalAccounts() {
  ...
  useEffect(() => {
    const fetchData = async () => {
        const { response, error } = await getDeletedUsers();
        if (response) {
  ...
  // Paginate data based on the current page and page size
      ...
      >
        <div className="flex flex-col justify-center items-center">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-[#645D5D] text-[14px] font-[400]">You have successfully restored the account</p>
          </div>
        </div>
      </Modal>
